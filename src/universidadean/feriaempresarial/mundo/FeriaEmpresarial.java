...
public class FeriaEmpresarial {
    ...
    public String metodo1() {
        ...
        int norte=0,sur=0,oriente=0,occidente=0,centro=0;
        String respuesta1 = "Vacio";

        for (int i = 0; i < puestos.length; i++) {

            if (puestos[i].estaOcupado()&&puestos[i].darZona()==Puesto.ZONA_NORTE){
                norte++;
            }else if(puestos[i].estaOcupado()&&puestos[i].darZona()==Puesto.ZONA_SUR){
                sur++;
            }else if(puestos[i].estaOcupado()&&puestos[i].darZona()==Puesto.ZONA_CENTRO){
                centro++;
            }else if(puestos[i].estaOcupado()&&puestos[i].darZona()==Puesto.ZONA_ORIENTE){
                oriente++;
            }else if(puestos[i].estaOcupado()&&puestos[i].darZona()==Puesto.ZONA_OCCIDENTE){
                occidente++;
            }
        }
        int maximo = Math.max(Math.max(Math.max(Math.max(norte,sur),oriente),occidente),centro);

        if (maximo==0){
            respuesta1 = "Vacio";
        }else if (norte==maximo){
            respuesta1 = Puesto.ZONA_NORTE;
        }else if (sur==maximo){
            respuesta1 = Puesto.ZONA_SUR;
        }else if (centro==maximo){
            respuesta1 = Puesto.ZONA_CENTRO;
        }else if (oriente==maximo){
            respuesta1 = Puesto.ZONA_ORIENTE;
        }else if (occidente==maximo){
            respuesta1 = Puesto.ZONA_OCCIDENTE;
        }
        return respuesta1;
    }
